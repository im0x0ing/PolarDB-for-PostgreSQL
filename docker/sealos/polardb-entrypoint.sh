...
docker_create_db_directories() {
	...
	if [ -n "${POLARDB_INITDB_WALDIR:-}" ]; then
		mkdir -p "$POLARDB_INITDB_WALDIR"
		if [ "$user" = '0' ]; then
			find "$POLARDB_INITDB_WALDIR" \! -user postgres -exec chown postgres '{}' +
		fi
		chmod 700 "$POLARDB_INITDB_WALDIR"
	fi
...
# arguments to `initdb` can be passed via POLARDB_INITDB_ARGS or as arguments to this function
# `initdb` automatically creates the "postgres", "template0", and "template1" dbnames
# this is also where the database user is created, specified by `POLARDB_USER` env
docker_init_database_dir() {
	...
	if [ -n "${POLARDB_INITDB_WALDIR:-}" ]; then
		set -- --waldir "$POLARDB_INITDB_WALDIR" "$@"
	fi

	# --pwfile refuses to handle a properly-empty file (hence the "\n"): https://github.com/docker-library/postgres/issues/1025
	eval 'initdb --username="$POLARDB_USER" --pwfile=<(printf "%s\n" "$POLARDB_PASSWORD") '"$POLARDB_INITDB_ARGS"' "$@"'
...
# print large warning if POLARDB_PASSWORD is long
# error if both POLARDB_PASSWORD is empty and POLARDB_HOST_AUTH_METHOD is not 'trust'
# print large warning if POLARDB_HOST_AUTH_METHOD is set to 'trust'
# assumes database is not set up, ie: [ -z "$DATABASE_ALREADY_EXISTS" ]
docker_verify_minimum_env() {
	if [ -z "$POLARDB_PASSWORD" ] && [ 'trust' != "$POLARDB_HOST_AUTH_METHOD" ]; then
		# The - option suppresses leading tabs but *not* spaces. :)
		cat >&2 <<-'EOE'
			Error: Database is uninitialized and superuser password is not specified.
			       You must specify POLARDB_PASSWORD to a non-empty value for the
			       superuser. For example, "-e POLARDB_PASSWORD=password" on "docker run".

			       You may also use "POLARDB_HOST_AUTH_METHOD=trust" to allow all
	...
	if [ 'trust' = "$POLARDB_HOST_AUTH_METHOD" ]; then
		cat >&2 <<-'EOWARN'
			********************************************************************************
			WARNING: POLARDB_HOST_AUTH_METHOD has been set to "trust". This will allow
			         anyone with access to the Postgres port to access your database without
			         a password, even if POLARDB_PASSWORD is set. See PostgreSQL
			         documentation about "trust":
			         https://www.postgresql.org/docs/current/auth-trust.html
			         In Docker's default configuration, this is effectively any other
			         container on the same system.

			         It is not recommended to use POLARDB_HOST_AUTH_METHOD=trust. Replace
			         it with "-e POLARDB_PASSWORD=password" instead to set a password in
			         "docker run".
			********************************************************************************
		EOWARN
	fi
...
# uses environment variables for input: POLARDB_DB
docker_setup_db() {
	local dbAlreadyExists
	dbAlreadyExists="$(
		POLARDB_DB= docker_process_sql --dbname postgres --set db="$POLARDB_DB" --tuples-only <<-'EOSQL'
			SELECT 1 FROM pg_database WHERE datname = :'db' ;
		EOSQL
	)"
	if [ -z "$dbAlreadyExists" ]; then
		POLARDB_DB= docker_process_sql --dbname postgres --set db="$POLARDB_DB" <<-'EOSQL'
			CREATE DATABASE :"db" ;
		EOSQL
		printf '\n'
	fi
...
docker_setup_env() {
	file_env 'POLARDB_PASSWORD'

	file_env 'POLARDB_USER' 'postgres'
	file_env 'POLARDB_DB' "$POLARDB_USER"
	file_env 'POLARDB_INITDB_ARGS'
	: "${POLARDB_HOST_AUTH_METHOD:=}"
...
# append POLARDB_HOST_AUTH_METHOD to pg_hba.conf for "host" connections
...
_main() {
	...
	if [ "$1" = 'postgres' ] && ! _pg_want_help "$@"; then
		...
		if [ -z "$DATABASE_ALREADY_EXISTS" ]; then
			...
			export PGPASSWORD="${PGPASSWORD:-$POLARDB_PASSWORD}"
			docker_temp_server_start "$@"
